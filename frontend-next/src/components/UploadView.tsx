...
function getFileIcon(file: File): string {
  ...
  if (file.type.startsWith('video/')) return '\u{1F3AC}';
  if (file.type.startsWith('audio/')) return '\u{1F3B5}';
  return '\u{1F4C4}';
}
...
export default function UploadView({ onStartProcessing }: Props) {
  ...
  const uploadFile = () => {
    ...
    const backendUrl = process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:9100';
      ...
      {selectedFile && (
        <div className="upload-meta">
          ...
          <button className="btn-analyze" onClick={uploadFile} disabled={uploading}>
            {uploading ? `${uploadPct}% uploaded` : (
              <>Begin Analysis <span className="btn-arrow">&rarr;</span></>
            )}
          </button>
        </div>
      )}
