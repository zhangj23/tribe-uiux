...
- **Backend:** Python / FastAPI (port 9100) — API only
- **Frontend:** Next.js 15 / React 19 (port 3000) — served separately
...
1. **Upload** — User uploads media via `POST /api/upload` → backend saves file, creates job (`upload.py`, `job_manager.py`)
...
7. **Display** — Next.js frontend polls for results and shows 3D brain heatmap, metrics, timeseries, and LLM analysis
...
backend/
  app/
    ...
    services/
      ...
      tribe_runner.py  — TRIBE v2 inference (mock or real)
...
frontend-next/         — Next.js 15 + React 19 frontend (primary)
  src/
    app/
      layout.tsx       — Root layout with metadata
      page.tsx         — Main page: view state management (upload → processing → results)
    components/
      Header.tsx       — Status indicator header (mock/live mode)
      UploadView.tsx   — File upload with drag-drop and progress bar (XMLHttpRequest)
      ProcessingView.tsx — Progress display during analysis pipeline
      ResultsView.tsx  — 3-panel results dashboard with playback + copy + metric toggle
      BrainCanvas.tsx  — 3D brain mesh rendering (Canvas 2D, rotatable, tooltips, labels)
      ProcessingCanvas.tsx — Animated brain during processing
      MetricGauges.tsx — 6 neural metric Z-score displays
      TimeseriesChart.tsx — Chart.js line chart of metrics over time
      FrictionScore.tsx — 1-10 friction score display
      AnalysisText.tsx — LLM-generated analysis renderer (XSS-safe)
    hooks/
      usePolling.ts    — Poll job status every 2 seconds
      useHealth.ts     — Check backend health on mount
    types/
      index.ts         — TypeScript interfaces (Job, ZScores, Timeseries)
    styles/
      globals.css      — Full design system (dark clinical neuroscience theme)
  next.config.ts       — API proxy rewrites to backend
  Dockerfile           — Node 22 Alpine container

frontend/              — Legacy vanilla HTML/CSS/JS frontend (not actively used)
...
Source files are volume-mounted, so edits to `backend/app/` and `frontend-next/src/` are reflected immediately (hot reload on both services).
...
Requires Python 3.12+, Node 22+, and ffmpeg installed on your system.
...
- `TRIBE_DEVICE` — `cuda` (default) or `cpu` for TRIBE inference.
- `NEXT_PUBLIC_BACKEND_URL` — Frontend env var for backend URL (default: `http://localhost:9100`). In Docker, the Next.js rewrite proxy uses `BACKEND_URL=http://app:9100` for small requests; uploads go directly to the backend to avoid Next.js body size limits.
...
- Backend uses FastAPI with async endpoints; pipeline runs in a background thread
- Frontend is Next.js 15 + React 19 with TypeScript — components in `frontend-next/src/components/`
...
- Brain visualization uses Canvas 2D with a 3D mesh (UV-sphere deformed into brain shape, painter's algorithm, Lambert lighting)
- Uploads use XMLHttpRequest (not fetch) to support progress events for large files
