...
- **Backend:** Python / FastAPI (port 9100)
...
7. **Display** — Frontend polls for results and shows heatmap, metrics, timeseries, and LLM analysis (`app.js`, `polling.js`, `brain-view.js`, `results.js`)
...
backend/
  app/
    ...
    main.py            — FastAPI app setup, middleware, routes, static file serving
...
# App serves at http://localhost:9100
```
...
Source files are volume-mounted, so edits to `backend/app/` and `frontend/` are reflected immediately (uvicorn has `reload=True`).
...
- Frontend is vanilla JS (no framework) — keep it that way
